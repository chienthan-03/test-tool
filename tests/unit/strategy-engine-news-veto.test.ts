...
import { wireNewsVeto } from '../../src/strategy/wire-news-veto.js';
...
const makeOpposingMacroSignal = (symbol: string): NewsSignal => ({
  id: 'sig-veto-1',
  newsId: 'news-macro-1',
  symbols: [symbol],
  direction: 'short',
...
const runTechnicalCandleClose = async (options: {
  config: AppConfig;
  emitOpposingSignal?: boolean;
}): Promise<TradeIntent[]> => {
  const { config, emitOpposingSignal = false } = options;
  ...
  if (emitOpposingSignal) {
    bus.emit('news:signal', makeOpposingMacroSignal(symbol));
  }
...
describe('StrategyEngine news veto on technical path', () => {
  let profileConfig: AppConfig;
  let vetoConfig: AppConfig;
  ...
  beforeAll(() => {
    ...
    vetoConfig = {
      ...
      strategy: {
        ...
        newsVeto: {
          ...profileConfig.strategy.newsVeto,
          enabled: true,
        },
      },
    };
  ...
  it('blocks strategy:intent when opposing news:signal registered before candle', async () => {
    const intents = await runTechnicalCandleClose({
      config: vetoConfig,
      emitOpposingSignal: true,
    });

    expect(intents).toHaveLength(0);
  });

  it('emits strategy:intent when no veto signal is active', async () => {
    const intents = await runTechnicalCandleClose({
      config: vetoConfig,
      emitOpposingSignal: false,
    });

    expect(intents.length).toBeGreaterThan(0);
    expect(intents[0]?.newsId).toBe('technical');
